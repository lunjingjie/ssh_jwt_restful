...
import com.ssh.jwt.redis.RedisFactory;
...
public class TokenUtil {
    ...
    public static String createToken(String tokenType, String userName) throws Exception {
        ...
        // TODO 使用redis缓存服务替换，存入token
        Jedis jedis = RedisFactory.getInstance();
        jedis.set(uuid, token);
    ...
    public static boolean verifyIsContains(String token) {
        try {
            ...
            // TODO 使用redis缓存服务替换，获取token
            if (RedisFactory.getInstance().get(tokenId) == null) {
                return false;
            }
        } catch (Exception e) {
            return false;
